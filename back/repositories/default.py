...
from constants import Prefixies_SPARQL, NamedGraph, HeadersOnRequests
# import Prefixies, NameSpaces as ns, Endpoint, EndpointDEV, Headers, Functions, NamedGraph
# from commons import ENVIROMENT, TBOX_SAVED_QUERY, Metadata_Graph_Tbox as VSKG
...
from utils import delete_file_in_the_graphdb_server
...
linha = len(TXT_TEN_DASHES + " REPOSITORY DEFAULT " + TXT_TEN_DASHES)
console = lambda x: f"\n{'-'*linha}\n{TXT_TEN_DASHES} REPOSITORY DEFAULT {TXT_TEN_DASHES}\n{'-'*linha}\n+ {x}"
...
class DefaultGraphRepository:
	def __init__(self, repository:str, language:str | None): 
		self.this = "DefaultGraphRepository"
	...
	def create_resource(self, query:dict, classe:str, label:str):
		"""Função genérica para criar um recurso no grafo default.
		
		Parameters
		----------
		query : dict
			dicionário {'update': str} com consulta SPARQL em str.
		classe : str 
			uma classe RDF/OWL no estilo "prefix:ClassName". Exemplo "foaf:Person".
      label : str
			o rótulo do recurso.
		...
		try:
			...
			if (existe["message"] == "Existe"):
				raise HTTPException(
					...
					detail="Um recurso dessa classe com esse label já existe!")
	...
	def retrieve_resources(self, query):
		"""Função genérica para recuperar recursos no grafo default.
